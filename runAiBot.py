            ...
            if overwrite_previous_answers or selected_option == "Select an option":
                ...
                # Add location handling
                    ...
                    if "current" in label or "present" in label:
                        answer = (
                            current_ctc if "month" not in label else current_ctc_monthly
                        )
                    else:
                        answer = (
                            desired_salary
                            if "month" not in label
                            else desired_salary_monthly
                        )
                    ...
                    answer = full_name
                elif "website" in label or "portfolio" in label or "link" in label:
                    answer = website
        ...
        if radio:
            ...
            if overwrite_previous_answers or prev_answer is None:
                if "citizenship" in label or "employment eligibility" in label:
                    ...
                    answer = current_ctc if "current" in label else desired_salary
                elif "notice" in label:
                    if "month" in label:
                        answer = notice_period_months
                    elif "week" in label:
...
# Function to pick the right resume for a job based on title keywords
def pick_resume_for_job(job_title: str) -> str | None:
    for keywords, path in resume_rules:
        for kw in keywords:
            if kw.lower() in job_title.lower():
                abs_path = os.path.abspath(path)
                if os.path.exists(abs_path):
                    return abs_path
    if os.path.exists(os.path.abspath(default_resume_path)):
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    resume = "Pending"
                    resume_path = pick_resume_for_job(title)
                    if resume_path:
                    ...
                    if is_easy_apply:
                        try:
                            try:
                                ...
                                # resume = "Previous resume" -- now handled by resume_path above
                                next_button = True
                                questions_list = set()
                                next_counter = 0
                                while next_button:
                    ...
                    # Resume upload state resets per job (handled by resume_path)
...
def main() -> None:
    ...
    total_runs = 1
    try:
        global linkedIn_tab, tabs_count, aiClient
        alert_title = "Error Occurred. Closing Browser!"
        validate_config()

        if not os.path.exists(default_resume_path):
            pyautogui.alert(
                text='Your default resume "{}" is missing! Please check your resume paths in config/questions.py (default_resume_path or resume_rules).\n\n\nFor now the bot will continue using your previous upload from LinkedIn!'.format(
                    default_resume_path
                ),
